...
from mlp import mlp_train_and_evaluate
from cnn import cnn_train_and_evaluate
...
def main(parse_args, configs):
    ...
    if parse_args.mode == 'predict':
        if parse_args.train_feature_path is not None and parse_args.test_feature_path is not None:
            ...
            mlp_train_and_evaluate(configs, parse_args.train_feature_path, parse_args.test_feature_path, use_smi=False, log_path=log_path)
            printl(f"{'=' * 128}", log_path=log_path)
            printl(f"MLP model training & binding specificity prediction - with additional SMI features", log_path=log_path)
            mlp_train_and_evaluate(configs, parse_args.train_feature_path, parse_args.test_feature_path, use_smi=True, log_path=log_path)
            printl(f"{'=' * 128}", log_path=log_path)
            printl(f"CNN model training & binding specificity prediction", log_path=log_path)
            cnn_train_and_evaluate(configs, parse_args.train_feature_path, parse_args.test_feature_path, use_smi=False, log_path=log_path)
            printl(f"{'=' * 128}", log_path=log_path)
            printl(f"CNN model training & binding specificity prediction - with additional SMI features", log_path=log_path)
            cnn_train_and_evaluate(configs, parse_args.train_feature_path, parse_args.test_feature_path, use_smi=True, log_path=log_path)
            return
        else:
